...
def finalize_and_file(file_path_or_slug: str, inbox_type: str, confirmed_yaml: str | None = None) -> str:
    ...
    if inbox_type == "journal":
        ...
        prompt = (
            ...
            f"2. Rename the file to a clean descriptive slug based on the source_name "
            f"field. Use kebab-case, no dates, no timestamps. Examples: "
            f"'salesforce-q4-fy2026-earnings.md', 'jbi-holdout-trade.md', "
            f"'iran-hormuz-deadline-extension.md'. "
            f"For PDFs/binary files, rename both the binary and its sidecar .md.\n"
            f"3. Move the renamed file from its current inbox location to {dest_desc}.\n"
            f"4. Make sure to-tag/ is empty for this file after the move.\n"
            f"5. Git commit: 'v2-ingest: <source-name> — {inbox_type} raw filed'\n"
            f"6. Return a brief confirmation: final path, key fields, commit hash. "
            f"Under 80 words."
        )
